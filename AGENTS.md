...
4. Display estimated reading time remaining for both the current chapter and the whole book, for example **18 mins left in chapter** and **9 hrs 15 mins left in book**. Make chapter and book estimates separately configurable, with both enabled by default. Add independent checkboxes for displaying the enabled estimates in the expanded reader controls and in the status bar, so either location can be used without the other. Base the estimates on actual remaining word counts, not page counts, provide an option to change the reading speed in words per minute (WPM), and do not make a chapter result wait for a full-book text scan. For EPUB, count the spine/XHTML text directly instead of scanning reflowed layout pages.
