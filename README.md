...
- Separately optional chapter and whole-book reading-time estimates based on actual remaining words, with configurable reading speed in words per minute. Both expanded-control estimates default on, while the status-bar copy is independently optional. EPUB estimates use spine/XHTML text rather than scanning reflowed layout pages; fixed-layout documents use direct native text-word counts rather than HTML serialization.
